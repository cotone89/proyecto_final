<template>
    <div class="busqueda">
        ...
        <div class="input-group input-group-lg busqueda__input">
            <input v-model="input" @keyup.enter="obtieneRecetaNombre()" type="text" class="form-control" placeholder="ej:pasta" aria-label="Large" aria-describedby="inputGroup-sizing-sm">
        </div>
        <button type="button" class="btn btn-success busqueda__boton mb-4 mt-4"  @click="obtieneRecetaNombre()">{{titulo_boton}}</button>

        <div id="resultadoBusqueda">

            <div id="busquedaNoEncontrada" class="busqueda__titulo__receta p-4" :style="{display:display}">
                ...
                <div class="row pb-3" style="margin:2%;">
                    ...
                    <div class="col-sm-6 p-3">
                        ...
                        <div class="busqueda__preparacion__receta text-left">
                            <ul>
                                <li v-for="(ingrediente,index) in receta_lista_ingredientes" :key="index">{{ ingrediente }}</li>
                            </ul>

                        </div>
                        <h3 class="busqueda__titulo__preparacion__receta">{{receta_titulo_preparacion}}</h3>
                        <div class="busqueda__preparacion__receta">
                            {{receta_preparacion}}
                        </div>
                    </div>
                </div>
            ...
            <div id="busquedaEncontradaVariosElementos" :style="{display:display3}">
                ...
                <div class="pb-4">
                    <b-container>
                        <b-row>
                            <b-col md="4" class="p-2 busqueda__lista__receta" @click="elijoReceta(receta)" v-for="(receta,index) in lista_recetas" :key="index">{{ receta.strMeal }}</b-col>
                        </b-row>
                    </b-container>
                </div>
            </div>
...
export default {
    ...
    data: function () {
        ...
        methods: {
            ...
            muestraReceta(retorno) {
                ...
                lista.forEach(element => {
                    if ((element != " ") && (element.trim() != "") && (!element.includes("null"))) {
        ...
        async obtieneRecetaNombre() {
            ...
            let retorno = await this.$store.dispatch('obtenerRecetaNombre', this.input)
